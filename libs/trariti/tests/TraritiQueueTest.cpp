...
int main() {
  ...
  uint64_t data = 419;
  auto res = trariti::enqueue<trariti::SyncType::SINGLE_THREAD,
                              trariti::Behavior::VARIABLE>(
      tr, reinterpret_cast<void *>(&data));
  ...
  uint64_t data_2 = 813;
  res = trariti::enqueue<trariti::SyncType::SINGLE_THREAD,
                         trariti::Behavior::VARIABLE>(
      tr, reinterpret_cast<void *>(&data_2));
  void *read_data = 0;
  res = trariti::dequeue<trariti::SyncType::SINGLE_THREAD,
                         trariti::Behavior::VARIABLE>(tr, read_data);
